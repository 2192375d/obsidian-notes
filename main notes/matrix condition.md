...
$$
\text{if }Az=y,\text{ then }\frac{||z||}{||y||}\leq||A^{-1}||
$$
We want to pick a $y$ S.T. $\displaystyle \frac{||z||}{||y||}$ has the largest ratio possible. A cheap way of doing this is to choose $y$ from the system $A^{T}y=c$ where $c$ is a vector with component vectors $\pm1$